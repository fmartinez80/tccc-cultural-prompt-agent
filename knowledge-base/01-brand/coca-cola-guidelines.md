...
**Spain — a confirmed, market-specific on-premise glass-bottle standard, distinct from the generic 330mL contour bottle above.** Found during the Spain country-file build, verified via Coca-Cola Europacific Partners Iberia's own current hostelería (on-premise/horeca) trade campaign, not a generic packaging-dimension source: **Coca-Cola's returnable glass bottle for Spanish bars and restaurants is sold in two sizes, 350mL (for meals) and 237mL (for an aperitivo/afternoon/evening drink)** — a real, named, current commercial program ("vidrio retornable," refillable up to 25 times), not a discontinued or regional curiosity. [CONFIDENCE: MEDIUM-HIGH for the two sizes and their meal-vs-aperitivo positioning — corroborated across multiple current Spanish trade-press articles describing the same CCEP Iberia campaign] [SOURCE: [El Español — Una botella de vidrio retornable, 25 vidas](https://www.elespanol.com/invertia/empresas/20260513/botella-vidrio-retornable-vidas-apuesta-coca-cola-hosteleria/1003744233868_32.html); [Revista Alimentaria — Coca-Cola refuerza su compromiso con la hostelería española](https://revistaalimentaria.es/horeca/bebidas/coca-cola-refuerza-su-compromiso-con-la-hosteleria-a-traves-de-su-campana-de-vidrio-retornable)]
- **350mL bottle**: 147mm tall, 76.1mm diameter — shorter and notably wider than the generic 200–210mm/60–65mm contour-bottle figures above, since this is a squatter returnable-bottle shape, not the tall slender contour silhouette. [CONFIDENCE: MEDIUM — a specific, repeated commercial-product spec found via WebSearch, not independently cross-checked against TCCC's own bottler documentation]
- **237mL bottle**: sources converge less precisely — figures range from roughly 188mm tall/56mm diameter to roughly 190–200mm tall/55–65mm diameter (this size is explicitly modeled on the historic 1915 contour-bottle design, so it reads closer to the classic tall-necked silhouette than the 350mL bottle does). [CONFIDENCE: LOW-MEDIUM for the exact figure — treat the shape as "close to a small classic contour bottle" and the height as roughly 19cm until a firmer single source is found, rather than locking a precise number]
- **When a Spain-set scene calls for an on-premise glass bottle rather than a can**, use these two sizes rather than the generic 330mL contour-bottle default above — this is exactly the kind of "different confirmed local standard" the non-US-market rule above anticipates. The 330mL can figures (115.2mm/66.1mm) remain correct for any can-based scene in Spain; this note affects glass-bottle scenes only.
